...
- [X] T015 实现 `ImportGeometryOperator`、`OperatorsModel::TaskService` 和 `OperatorsGUI` 路由 | 前置任务: T010, T012, T014 | 影响模块: OperatorsModel、OperatorsGUI | 需求编号: FR-006, FR-009, FR-015 | 设计章节: 系统设计报告第 6.1、14.4 节 | 输入: 文件路径、工作目录、几何 IO 适配器 | 输出: 异步导入任务 | 验收条件: 成功后原子提交 GeometryObject；失败不改变工程并显示诊断；有界 stop 可诊断超时，最终 drain/析构等待所有已接受任务并保证业务依赖存活，生产 MeshApp 生命周期使用真实 TaskService 边界
- [X] T016 [P] [S4] 接入首版必选几何格式适配器 `src/io/GeometryIO/` 并声明格式能力 | 前置任务: T015 | 影响模块: GeometryIO、FITK 适配 | 需求编号: FR-006, FR-012 | 设计章节: plan.md Module Boundaries; spec.md Clarifications | 输入: 已冻结的 BRep/STEP/STP/IGES/IGS 清单 | 输出: 格式注册、读取和校验适配器 | 验收条件: 清单内有效文件可导入；损坏/空文件/不支持版本有错误；清单变更同步测试夹具
- [X] T017 [P] [S4] 编写几何导入单元、失败和端到端测试 `tests/integration/geometry/` | 前置任务: T015, T016 | 影响模块: OperatorsModel、GeometryIO、ModelData | 需求编号: FR-006, FR-020, SC-002, SC-008 | 设计章节: quickstart.md End-to-end checks、UI responsiveness heartbeat | 输入: 有效/无效/超内存文件 | 输出: 自动化验收结果 | 验收条件: 成功刷新树/视图；失败无残留；受控退出保留最后有效数据；真实几何导入后台运行至少 5 秒时满足 SC-002 的 100 ms、至少 50 样本、最大 500 ms、running 和单一终态合同
