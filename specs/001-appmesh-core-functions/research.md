...
**Decision**: T015 introduces `ITaskExecutor` and a production `FITKTaskExecutor` whose private `FITKThreadTask` runs one value-captured callable. The adapter tracks only its own pending callables in shared state and exposes both bounded waiting and final drain, so `TaskService` neither duplicates a thread pool nor blocks on unrelated FITK work. `TaskService` continues to own APPMesh `Task` snapshots and observer dispatch; concrete `IOperator` values are injected directly instead of pretending that `FITKOperatorRepo` supplies execution semantics it does not have. A rejecting test executor covers the explicit rejection path that the real FITK `void` API cannot report.

**Lifetime boundary**: FITK owns and auto-deletes each submitted task object. A fixed five-second wait cannot guarantee destruction safety because valid readers may run longer, so bounded `stop(timeoutMs)` is retained only for diagnostics. `TaskService` and `FITKTaskExecutor` destructors perform a non-throwing final drain after disabling submissions. The production `TaskServiceOperatorBoundary` connects that same service to MeshApp's `operations.stop` and `tasks.finish` stages. `MainWindowGenerator` destroys QWidget/Controller ownership before its cleanup callback releases TaskService, MeshManager and GeometryManager, so GlobalDataFactory cannot release ApplicationRuntime first; window recreation uses the same drain-and-release boundary. QPointer/RAII GUI subscriptions remain the callback lifetime guard. No detached thread, leak, second production pool, cancellation state or additional FITK binary is introduced.
...
No direct or transitive dependency of the T041 base pair names an original APPMesh business DLL. T016 deliberately expands the production Debug whitelist with `FITKInterfaceModel`, `FITKInterfaceGeometry` and `FITKGeoCompOCC`; all are the repository's current public FITK/OCC implementation and not original APPMesh business binaries. `FITKAdaptor`, `FITKRenderWindowVTK`, `FITKPython`, mesh, generator, AI, widget and solver/data modules remain outside the whitelist until their owning tasks.
...
The T041 baseline validation ran 12/12 CTest tests and 5/5 T004/T041 focused tests. The real non-blocking smoke process reports `FITKAppFramework=1.0.7`, `FITKCore=1.0.4`, the real adapter name and strict startup/reverse-shutdown logs. After T016, `APPMesh.exe` additionally directly depends on `FITKGeoCompOCC.dll` and `FITKInterfaceGeometry.dll`; `FITKInterfaceModel.dll` is their staged transitive dependency. It contains no Release Qt/HDF5 or original APPMesh business dependency.

## Decision: T016 uses the public FITKGeoCompOCC import command

`output/bin_d` contains matching Debug `.lib/.dll` pairs for `FITKInterfaceModel`, `FITKInterfaceGeometry` and `FITKGeoCompOCC`. Public source inspection confirms `OCC::FITKOCCModelImport::update()` invokes `BRepTools::Read`, `STEPControl_Reader` and `IGESControl_Reader`, then builds the public OCC shape agent. APPMesh therefore constructs this public command directly, never loads an original APPMesh GeometryIO binary, and advertises only BRep, STEP/STP and IGES/IGS although the FITK class supports additional formats.

The reader serializes the non-documented thread-safety portion of OCC import behind an adapter mutex, converts public FITK shape counts into value-only topology keys, and returns a transaction owning the FITK command. T017 validates the real GUI-routed five-format path with mutex-protected task-event observation, asynchronous failure isolation and APPMesh-first diagnostics. A real OCC post-read gate drives the 100 ms GUI heartbeat for at least five seconds while `stopAccepting()->drain()` blocks until accepted work completes. The lifecycle test snapshots the prior object/tree/view/repository state, verifies it after drain, and then proves ordered observer/service/manager/runtime destruction releases all repository models without late GUI callbacks or duplicate terminal events. The test observes only queued stable-ID placeholder-view refresh; GraphData/VTK actors remain deferred to T026-T028. The composition root registers all three real readers and treats registration conflict as startup failure.

The capability table is `brep -> .brep -> BRep`, `step (alias stp) -> .step/.stp -> Step`, and `iges (alias igs) -> .iges/.igs -> Iges`, all with `supportsRead=true` and `providerKey=fitk.occ`. Topology IDs are nonzero values local to the APPMesh geometry; adapter keys use `fitk-occ:<FITK data id>/<vertex|edge|face|solid>:<zero-based OCC index>`, so they remain stable while the repository owns that command.

`dumpbin /dependents` requires the Debug FITK pair above plus `FITKInterfaceModel`, the audited OCC runtime closure (`TKBO`, `TKBool`, `TKBRep`, `TKernel`, `TKFeat`, `TKFillet`, `TKG2d`, `TKG3d`, `TKGeomAlgo`, `TKGeomBase`, `TKIGES`, `TKLCAF`, `TKMath`, `TKMesh`, `TKOffset`, `TKPrim`, `TKShHealing`, `TKSTEP`, `TKSTL`, `TKTopAlgo`, `TKXCAF`, `TKXDEIGES`, `TKXDESTEP`, `TKXSBase`) and `dime0d`. CMake stages this explicit list only; it does not scan or copy all of `output/bin_d`.
