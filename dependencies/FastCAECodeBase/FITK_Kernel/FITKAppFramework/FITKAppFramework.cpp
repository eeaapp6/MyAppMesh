...
#include "FITK_Kernel/FITKCore/FITKGraphObjRepo.h"
...
namespace AppFrame
{
    ...
    FITKApplication::~FITKApplication()
    {
        ...
        if (_programTaskManager){delete _programTaskManager; _programTaskManager = nullptr; }
        if (_signalTransfer) {delete _signalTransfer; _signalTransfer = nullptr; }
        if (_pluginManager) {delete _pluginManager; _pluginManager = nullptr; }

        if (_golbalData) {delete _golbalData; _golbalData = nullptr; }
        if (_components) {delete _components; _components = nullptr; }
        if (_gDataFactory) {delete _gDataFactory; _gDataFactory = nullptr; }
        if (_componentsFactory) {delete _componentsFactory; _componentsFactory = nullptr; }
        if (_mwGenerator) {delete _mwGenerator; _mwGenerator = nullptr; }
        if (_globalEvent){delete _globalEvent; _globalEvent = nullptr; }
        if (_commandLineHandler) {delete _commandLineHandler; _commandLineHandler = nullptr; }
        if (_workbenchHandler) {delete _workbenchHandler; _workbenchHandler = nullptr; }
        if (_settings) {delete _settings; _settings = nullptr; }
        if (_pyReg) {delete _pyReg; _pyReg = nullptr; }
        if (_autoSaver) {delete _autoSaver; _autoSaver = nullptr; }
        if (_initializer) {delete _initializer; _initializer = nullptr; }
        if (_scriptHelper) {delete _scriptHelper; _scriptHelper = nullptr; }
        if (_licenseChecker) {delete _licenseChecker; _licenseChecker = nullptr; }

        Core:: FITKOperatorRepo::getInstance()->clear();
        Core:: FITKGraphObjRepo::getInstance()->clear();
    }
 ...
 //          this->showCopyrightDlg();
    ...
    bool FITKApplication::init()
    {
        ...
        if (_mwGenerator->showMaximize())
        {
            ...
            if (w->windowFlags().testFlag(Qt::FramelessWindowHint))
            {
