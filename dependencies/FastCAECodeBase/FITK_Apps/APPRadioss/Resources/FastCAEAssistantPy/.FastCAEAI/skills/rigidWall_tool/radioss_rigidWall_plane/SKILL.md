...
本技能用于在Radioss中创建和修改平面刚性墙，支持设置刚性墙名称、滑移类型、搜索距离、摩擦系数、滤波参数、节点组以及平面几何参数。

## 使用方法

通过Python语言接口调用，使用submitPythonCommand执行平面刚性墙配置脚本。
...
根据刚性墙名称获取已存在的平面刚性墙对象：
- 参数：刚性墙名称（字符串）
- 编辑前可先通过 `AI.AI_Solution().getRWallType('RigidWall-1')` 判断对象是否为 `RWallPlane` 类型，如果不是则不在这个skill中编辑，查找别的skill。
...
| 方法 | 参数类型 | 说明 |
|------|----------|------|
| RWallPlane | - | 创建RWallPlane对象 |
| RWallPlane.GetRWallPlane | string | 根据名称获取已存在的平面刚性墙对象 |
| createRWall | string | 创建指定名称的平面刚性墙 |
...
| setBasicPoint | double, double, double | 设置平面基点坐标 |
| setNormalVector | double, double, double | 设置平面法向量 |

## 注意事项

1. 默认使用当前有效的Solution，无需额外判断或设置。
2. 同名平面刚性墙已存在时，`createRWall` 会失败。
3. 编辑前建议先通过 `RWall.RWallPlane.GetRWallPlane` 获取对象。
4. 编辑已有刚性墙时，建议先通过 `AI.AI_Solution().getRWallType('刚性墙名')` 确认对象类型。
5. `setNodeGroup1('None')` 和 `setNodeGroup2('None')` 表示清除对应节点组设置。
6. 当前界面 `writePythonScript` 仅在滑移类型为带摩擦时输出 `setFrictionCoef`。
7. 平面基点和法向量共同决定平面刚性墙的空间位置与方向。
