...
{
    /**
     * @brief OperPHengLEIManagerBase的构造函数
     *
     * 该构造函数初始化OperPHengLEIManagerBase类的一个实例。它通过动态类型转换从FITKAPP的全局数据中获取GUI::MainWindow的指针，并将其赋值给_mainWindow成员变量。
     *
     * @param args 构造函数参数列表（当前未列出具体参数，表示可能有可选的参数传递给构造函数）
     */
    OperPHengLEIManagerBase::OperPHengLEIManagerBase(/* args */)
    {
        _mainWindow = dynamic_cast<GUI::MainWindow *>(FITKAPP->getGlobalData()->getMainWindow());
    }

    /**
     * @brief OperPHengLEIManagerBase的析构函数
     *
     * 该析构函数释放OperPHengLEIManagerBase类实例的资源，是一个空函数体。
     */
    OperPHengLEIManagerBase::~OperPHengLEIManagerBase()
    {

    }

    void OperPHengLEIManagerBase::preArgs()
    {
