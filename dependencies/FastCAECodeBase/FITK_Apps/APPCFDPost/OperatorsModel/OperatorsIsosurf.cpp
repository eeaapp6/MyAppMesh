...
namespace OperModel
{
    OperatorsIsosurf::OperatorsIsosurf()
    {

    }

    bool OperatorsIsosurf::execGUI()
    {
        ...
        //获取树节点当前数据对象id
        if (_senderName == "actionIsosurfCreate") {
            GUI::IsosurfDialog* dialog = new GUI::IsosurfDialog(treeWidget->getCurrentDataID(), this, true);
            dialog->show();
        }
        else if (_senderName == "actionIsosurfEdit") {
            int objID = -1;
            argValue(objID_String, objID);
            if (objID <= 0)return false;

            GUI::IsosurfDialog* dialog = new GUI::IsosurfDialog(objID, this, false);
            dialog->show();
        }
        return false;
    }

    bool OperatorsIsosurf::execProfession()
    {
        Interface::FITKCFDPost3DManager* manager = FITKAPP->getGlobalData()->getPostData<Interface::FITKCFDPostData>()->getPostDataManager();
        if (manager == nullptr)return false;
