...
#include "OperatorsStreamLine.h"

#include "GUIDialog/StreamLineDialog.h"
...
namespace OperModel
{
    bool OperatorsStreamLine::execGUI()
    {
        ...
        if (_senderName == "actionStreamlineCreate") {
            GUI::StreamLineDialog* dialog = new GUI::StreamLineDialog(treeWidget->getCurrentDataID(), this, true);
            dialog->show();
        }
            ...
            GUI::StreamLineDialog* dialog = new GUI::StreamLineDialog(objID, this, false);
            dialog->show();
        }
        return false;
    }

    bool OperatorsStreamLine::execProfession()
    {
