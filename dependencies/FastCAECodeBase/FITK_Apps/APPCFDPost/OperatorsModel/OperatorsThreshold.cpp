...
#include "OperatorsThreshold.h"

#include "GUIDialog/ThresholdDialog.h"
...
namespace OperModel
{
    bool OperatorsThreshold::execGUI()
    {
        ...
        //获取树节点当前数据对象id
        if (_senderName == "actionThresholdCreate") {
            GUI::ThresholdDialog* dialog = new GUI::ThresholdDialog(treeWidget->getCurrentDataID(), this, true);
            dialog->show();
        }
        else if (_senderName == "actionThresholdEdit") {
            int objID = -1;
            argValue(objID_String, objID);
            if (objID <= 0)return false;

            GUI::ThresholdDialog* dialog = new GUI::ThresholdDialog(objID, this, false);
            dialog->show();
        }
        return false;
    }

    bool OperatorsThreshold::execProfession()
    {
