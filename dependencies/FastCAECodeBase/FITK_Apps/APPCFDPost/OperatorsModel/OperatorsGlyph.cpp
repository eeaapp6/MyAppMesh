...
namespace OperModel
{
    OperatorsGlyph::OperatorsGlyph()
    {

    }

    bool OperatorsGlyph::execGUI()
    {
        ...
        //获取树节点当前数据对象id
        if (_senderName == "actionGlyphCreate") {
            GUI::GlyphDialog* dialog = new GUI::GlyphDialog(treeWidget->getCurrentDataID(), this, true);
            dialog->show();
        }
        else if (_senderName == "actionGlyphEdit") {
            int objID = -1;
            argValue(objID_String, objID);
            if (objID <= 0)return false;

            GUI::GlyphDialog* dialog = new GUI::GlyphDialog(objID, this, false);
            dialog->show();
        }
        return false;
    }

    bool OperatorsGlyph::execProfession()
    {
