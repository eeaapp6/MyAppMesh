...
#include "SurfaceClosedWireInfoDialog.h"
#include "ui_SurfaceClosedWireInfoDialog.h"
...
namespace GUI {
    SurfaceClosedWireInfoDialog::SurfaceClosedWireInfoDialog(QWidget *parent) :
        GUIDialogBase(parent),
        m_Ui(new Ui::SurfaceClosedWireInfoDialog)
    {
        m_Ui->setupUi(this);
        // 设置初始状态
    ...
    SurfaceClosedWireInfoDialog::~SurfaceClosedWireInfoDialog()
    {
        finalize();
        delete m_Ui;
    }
    SurfaceClosedWireInfoDialog* SurfaceClosedWireInfoDialog::New(QWidget * parent, EventOper::ParaWidgetInterfaceOper * oper, Interface::FITKAbsGeoCommand* cmd)
    {
        auto command = dynamic_cast<Interface::FITKAbsGeoModelClosedWireSurface*>(cmd);

        static SurfaceClosedWireInfoDialog* uniqueObj;
        if (uniqueObj == nullptr) {
            uniqueObj = new SurfaceClosedWireInfoDialog(parent);
        }
        ...
        if (command == nullptr)
        {
            ...
            uniqueObj->m_SourceShapes = command->edges();
            uniqueObj->m_Ui->lb_source->setText(QString(tr("Picked (%1)")).arg(uniqueObj->m_SourceShapes.size()));
        }
        /*@}*/
        return uniqueObj;
    }
    bool SurfaceClosedWireInfoDialog::checkDialogData() const
    {
        // 数据检查
    ...
    void SurfaceClosedWireInfoDialog::finalize()
    {
        // 清除拾取状态
        if (m_GraphPickOper)
        {
    ...
    void SurfaceClosedWireInfoDialog::on_comb_type_currentIndexChanged(int index)
    {
        m_Ui->stackedWidget->setCurrentIndex(index);
    }

    void SurfaceClosedWireInfoDialog::on_pb_selectsource_clicked()
    {
        // 设置拾取信息
    ...
    void SurfaceClosedWireInfoDialog::on_pb_ok_clicked()
    {
        // 检查输入
    ...
    void SurfaceClosedWireInfoDialog::on_pb_preview_clicked()
    {
        // 检查数据
        ...
        if (m_PreviewCmd == nullptr) {
            ...
            m_PreviewCmd = fac->createCommandT<Interface::FITKAbsGeoModelClosedWireSurface>(Interface::FITKGeoEnum::FITKGeometryComType::FGTClosedWireSurface);
        }
        // 更新预览对象数据
        m_PreviewCmd->setEdges(m_SourceShapes);
    ...
    void SurfaceClosedWireInfoDialog::on_pb_cancel_clicked()
    {
        // 结束对象操作。
        finalize();
