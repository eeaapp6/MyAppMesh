...
#include "PointCurveAndRadioInfoDialog.h"
#include "ui_PointCurveAndRadioInfoDialog.h"
...
namespace GUI {
    ...
    PointCurveAndRadioInfoDialog::PointCurveAndRadioInfoDialog(QWidget *parent) :
        GUIDialogBase(parent),
        m_Ui(new Ui::PointCurveAndRadioInfoDialog)
    {
        m_Ui->setupUi(this);
    ...
    PointCurveAndRadioInfoDialog::~PointCurveAndRadioInfoDialog()
    {
        finalize();
        delete m_Ui;
    }

    PointCurveAndRadioInfoDialog* PointCurveAndRadioInfoDialog::New(QWidget * parent, EventOper::ParaWidgetInterfaceOper* oper, Interface::FITKAbsGeoCommand* cmd)
    {
        auto command = dynamic_cast<Interface::FITKAbsGeoModelCurveRadio*>(cmd);

        static PointCurveAndRadioInfoDialog* uniqueObj;
        if (uniqueObj == nullptr) {
            uniqueObj = new PointCurveAndRadioInfoDialog(parent);
        }
        ...
        if (command == nullptr)
        {
            uniqueObj->m_Ui->lb_curveradio_picked->setText(tr("Picked (0)"));
            uniqueObj->m_Ui->le_curveradio_radio->setValue(0);
        }
        else {
            uniqueObj->m_dataId = command->getDataObjectID();
            uniqueObj->m_SourceCurve = command->sourceCurve();
            uniqueObj->m_Ui->lb_curveradio_picked->setText(uniqueObj->m_SourceCurve.isNull() ? tr("Picked (0)") : tr("Picked (1)"));
            uniqueObj->m_Ui->le_curveradio_radio->setValue(command->radio());
        }
        /*@}*/
    ...
    bool PointCurveAndRadioInfoDialog::checkDialogData()
    {
        // 数据检查
        if (m_SourceCurve.isNull() || m_Ui->le_curveradio_radio->text().isEmpty()) {
    ...
    void PointCurveAndRadioInfoDialog::finalize()
    {
        // 清除拾取状态
        if (m_GraphPickOper)
        {
    ...
    void PointCurveAndRadioInfoDialog::on_comb_type_currentIndexChanged(int index)
    {
        m_Ui->stackedWidget->setCurrentIndex(index);
    }

    void PointCurveAndRadioInfoDialog::on_pb_curveradio_pick_clicked()
    {
        // 设置拾取信息
        ...
        connect(pickDataProvider, &GraphData::GraphPickedDataProvider::sig_dataPicked, [this, pickDataProvider]() {
            ...
            m_Ui->lb_curveradio_picked->setText(m_SourceCurve.isNull() ? tr("Picked (0)") : tr("Picked (1)"));
        });
    }

    void PointCurveAndRadioInfoDialog::on_pb_ok_clicked()
    {
        // 检查输入
        ...
        m_Operator->setArgs("Ratio", m_Ui->le_curveradio_radio->value());
    ...
    void PointCurveAndRadioInfoDialog::on_pb_preview_clicked()
    {
        // 检查数据
        ...
        if (m_PreviewCmd == nullptr) {
            ...
            m_PreviewCmd = fac->createCommandT<Interface::FITKAbsGeoModelCurveRadio>(Interface::FITKGeoEnum::FITKGeometryComType::FGTCurveRadio);
        }
        // 更新预览对象数据
        m_PreviewCmd->setSourceCurve(m_SourceCurve);
        m_PreviewCmd->setRadio(m_Ui->le_curveradio_radio->value());
    ...
    void PointCurveAndRadioInfoDialog::on_pb_cancel_clicked()
    {
        // 结束对象操作。
        finalize();
