...
#include "SurfaceOffsetInfoDialog.h"
#include "ui_SurfaceOffsetInfoDialog.h"
...
namespace GUI {
    SurfaceOffsetInfoDialog::SurfaceOffsetInfoDialog(QWidget *parent) :
        GUIDialogBase(parent),
        m_Ui(new Ui::SurfaceOffsetInfoDialog)
    {
        m_Ui->setupUi(this);
        // 设置初始状态
    ...
    SurfaceOffsetInfoDialog::~SurfaceOffsetInfoDialog()
    {
        finalize();
        delete m_Ui;
    }
    SurfaceOffsetInfoDialog* SurfaceOffsetInfoDialog::New(QWidget * parent, EventOper::ParaWidgetInterfaceOper * oper, Interface::FITKAbsGeoCommand* cmd)
    {
        auto command = dynamic_cast<Interface::FITKAbsGeoModelOffsetSurface*>(cmd);

        static SurfaceOffsetInfoDialog* uniqueObj;
        if (uniqueObj == nullptr) {
            uniqueObj = new SurfaceOffsetInfoDialog(parent);
        }
        ...
        if (command == nullptr)
        {
            ...
            uniqueObj->m_Ui->le_offset->setValue(command->offset());
        }
        /*@}*/
        return uniqueObj;
    }

    bool SurfaceOffsetInfoDialog::checkDialogData() const
    {
        // 数据检查
        if (m_SourceShape.isNull() || m_Ui->le_offset->text().isEmpty()) {
    ...
    void SurfaceOffsetInfoDialog::finalize()
    {
        // 清除拾取状态
        if (m_GraphPickOper)
        {
    ...
    void SurfaceOffsetInfoDialog::on_comb_type_currentIndexChanged(int index)
    {
        m_Ui->stackedWidget->setCurrentIndex(index);
    }

    void SurfaceOffsetInfoDialog::on_pb_selectsource_clicked()
    {
        // 设置拾取信息
    ...
    void SurfaceOffsetInfoDialog::on_pb_ok_clicked()
    {
        // 检查输入
        ...
        m_Operator->setArgs("Offset", QVariant::fromValue(m_Ui->le_offset->value()));
    ...
    void GUI::SurfaceOffsetInfoDialog::on_pb_preview_clicked()
    {
        // 检查数据
        ...
        if (m_PreviewCmd == nullptr) {
            ...
            m_PreviewCmd = fac->createCommandT<Interface::FITKAbsGeoModelOffsetSurface>(Interface::FITKGeoEnum::FITKGeometryComType::FGTOffsetSurface);
        }
        // 更新预览对象数据
        m_PreviewCmd->setSourceSurface(m_SourceShape);
        m_PreviewCmd->setOffset(m_Ui->le_offset->value());
    ...
    void GUI::SurfaceOffsetInfoDialog::on_pb_cancel_clicked()
    {
        // 结束对象操作。
        finalize();
