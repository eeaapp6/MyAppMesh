...
#include "SolidExtrudeInfoDialog.h"
#include "ui_SolidExtrudeInfoDialog.h"
...
namespace GUI {
    SolidExtrudeInfoDialog::SolidExtrudeInfoDialog(QWidget *parent) :
        GUIDialogBase(parent),
        m_Ui(new Ui::SolidExtrudeInfoDialog)
    {
        m_Ui->setupUi(this);
    ...
    SolidExtrudeInfoDialog::~SolidExtrudeInfoDialog()
    {
        finalize();
        delete m_Ui;
    }
    SolidExtrudeInfoDialog* SolidExtrudeInfoDialog::New(QWidget * parent, EventOper::ParaWidgetInterfaceOper * oper, Interface::FITKAbsGeoCommand* cmd)
    {
        auto command = dynamic_cast<Interface::FITKAbsGeoModelExtrudeSolid*>(cmd);
        static SolidExtrudeInfoDialog* uniqueObj;
        if (uniqueObj == nullptr) {
            uniqueObj = new SolidExtrudeInfoDialog(parent);
        }
        ...
        if (command == nullptr)
        {
            ...
            auto dir = command->direction();
            uniqueObj->m_Ui->le_vx->setValue(dir[0]);
            uniqueObj->m_Ui->le_vy->setValue(dir[1]);
            uniqueObj->m_Ui->le_vz->setValue(dir[2]);
    ...
    bool SolidExtrudeInfoDialog::checkDialogData() const
    {
        // 数据检查
    ...
    void SolidExtrudeInfoDialog::finalize()
    {
        // 清除拾取状态
        if (m_GraphPickOper)
        {
    ...
    void SolidExtrudeInfoDialog::on_comb_type_currentIndexChanged(int index)
    {
        m_Ui->stackedWidget->setCurrentIndex(index);
    }

    void SolidExtrudeInfoDialog::on_pb_selectsource_clicked()
    {
        // 设置拾取信息
        ...
        connect(pickDataProvider, &GraphData::GraphPickedDataProvider::sig_dataPicked, [this, pickDataProvider]() {
            ...
            if (pickedList.size() == 1) {
                ...
                graphToolOper->setPickDirectionAnchorTopoId(m_SourceShape.CmdId, m_SourceShape.VirtualTopoId);
    ...
    void SolidExtrudeInfoDialog::onDirectionReverseSlot()
    {
        // 获取箭头方向
    ...
    void SolidExtrudeInfoDialog::on_pb_pickdirection_clicked()
    {
        // 设置拾取信息
        GUIPickInfoStru pickInfo{};
        pickInfo._pickMethod = GUI::GUIPickInfo::PickMethod::PMSingle;
    ...
    void SolidExtrudeInfoDialog::on_pb_ok_clicked()
    {
        // 检查输入
    ...
    void SolidExtrudeInfoDialog::on_pb_preview_clicked()
    {
        // 检查数据
        ...
        if (m_PreviewCmd == nullptr) {
            ...
            m_PreviewCmd = fac->createCommandT<Interface::FITKAbsGeoModelExtrudeSolid>(Interface::FITKGeoEnum::FITKGeometryComType::FGTExtrudeSolid);
        }
        // 更新预览对象数据
    ...
    void SolidExtrudeInfoDialog::on_pb_cancel_clicked()
    {
        // 结束对象操作。
        finalize();
