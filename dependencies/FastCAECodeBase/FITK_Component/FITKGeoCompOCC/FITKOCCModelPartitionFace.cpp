...
namespace OCC
{
    ...
    bool FITKOCCModelPartitionFaceWithDatumPlane::update()
    {
        ...
        // 分割面。
        BOPAlgo_Splitter splitter;
        splitter.AddArgument(*partShape);
        ...
        try
        {
            ...
            for (const TopoDS_Face* face : topoFaces)
            {
                ...
                // 面面求交。
                BRepAlgoAPI_Section section(*face, facePlane, Standard_False);
                section.Build();
                if (!section.IsDone())
                {
                    return false;
                ...
                TopExp_Explorer iter;
                for(iter.Init(section.Shape(), TopAbs_EDGE); iter.More(); iter.Next())
                {
                    const TopoDS_Edge & newEdge = TopoDS::Edge(iter.Current());
                    builder.Add(compound, newEdge);
                }
            }

            if (FITKOCCComandCommon::isEmpty(compound))
            {
                return false;
            }

            splitter.AddTool(compound);
            splitter.Perform();
        }
        catch (...)
        {
    ...
    bool FITKOCCModelPartitionFaceWithExtendFace::update()
    {
        ...
        try
        {
            ...
            for (const TopoDS_Face* face : topoFaces)
            {
                ...
                // 面面求交。
                BRepAlgoAPI_Section section(*face, faceExt, Standard_False);
                section.Build();
                if (!section.IsDone())
                {
                    return false;
    ...
    bool FITKOCCModelPartitionFaceWithIntersectFace::update()
    {
        ...
        try
        {
            ...
            for (const TopoDS_Face* face : topoFaces)
            {
                ...
                {
                    // 面面求交。
                    BRepAlgoAPI_Section section(*face, *faceTool, Standard_False);
                    section.Build();
                    if (!section.IsDone())
                    {
                        return false;
