...
namespace OCC
{
    ...
    bool FITKOCCModelChamferSolid::update()
    {
        ...
        // 获取体形状。
        //@{
        //OCC::FITKOCCTopoShape* vOCCShapeSolid = cmd->getShapeTopoByIndexT<OCC::FITKOCCTopoShape>(Interface::FITKGeoEnum::VTopoShapeType::VSSolid, m_solidVirtualIndex);
        //if (!vOCCShapeSolid)
        //{
        //    return false;
        //}
    ...
    bool FITKOCCDefeatureTool::Defeature(int cmdId, QList<int> faceIndice, TopoDS_Shape & outputShape)
    {
        ...
        try
        {
            QList<TopoDS_Shape> subShapes, subShapes2D, subShapes3D;
            FITKOCCComandCommon::FlattenCompShape2D3D(*shape, subShapes, subShapes2D, subShapes3D);
            ...
            shapes << s << subShapes << subShapes2D;
            FITKOCCComandCommon::MergeShapes(shapes, compound);
