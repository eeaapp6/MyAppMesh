...
namespace OCC
{
    ...
    bool FITKOCCEdgeTool::isPosOnEdge(Interface::FITKAbsVirtualTopo* vtp, double* iPos)
    {
        ...
        gp_Pnt pt(iPos[0], iPos[1], iPos[2]);
        Standard_Real u;
        bool isOnEdge = GeomLib_Tool::Parameter(curve, pt, 1e-3, u);
        if (!isOnEdge)
        {
            return false;
        }
    ...
    bool FITKOCCEdgeTool::sampleEdge(Interface::FITKAbsVirtualTopo* vtp, int iNPoses,
        QVector<QVector<double>> & oPoses, QVector<QVector<double>> & oDirs, int iModelId)
    {
        oPoses.clear();
        oDirs.clear();
        ...
        // 计算采样间隔。
        int nRealPoses = qMax(iNPoses, 2);
        double delta = (last - first) / nRealPoses;
        if (qFuzzyIsNull(delta))
        {
            return false;
        }

        for (int i = 0; i < nRealPoses; i++)
        {
            double iu = first + delta * i;
            ...
            {
                pos3[i] = pt.Coord(i + 1);
                dir3[i] = vec.Coord(i + 1);
            }

            if (model)
            {
                model->transformPoint(pos3, pos3);
                model->transformDirection(dir3, dir3);
            }

            oPoses.push_back(QVector<double>{ pos3[0], pos3[1], pos3[2] });
            oDirs.push_back(QVector<double>{ dir3[0], dir3[1], dir3[2] });
        }    
    ...
    bool FITKOCCFaceTool::getCylindSurfRadius(Interface::FITKAbsVirtualTopo* vtp, double & oRadius)
    {
        if (!vtp)
        {
            return false;
    ...
    bool FITKOCCFaceTool::getCylindSurfInfo(Interface::FITKAbsVirtualTopo* vtp, double* oCt, double* oDir, double & oRadius, double* oBds)
    {
        if (!vtp)
        {
            return false;
        ...
        Bnd_Box box;
        BRepBndLib::AddClose(face, box);
        box.Get(oBds[0], oBds[1], oBds[2], oBds[3], oBds[4], oBds[5]);
    ...
    bool FITKOCCFaceTool::getConeSurfInfo(Interface::FITKAbsVirtualTopo* vtp, double* oCt, double* oApex, double* oDir, double & oRadius, double* oBds)
    {
        if (!vtp)
        {
            return false;
        ...
        if (baSurf.GetType() == GeomAbs_SurfaceType::GeomAbs_Cone)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                oCt[i] = ct.Coord(i + 1);
                oDir[i] = dir.Coord(i + 1);
                oApex[i] = apex.Coord(i + 1);
            }

            oRadius = cone.RefRadius();
        }
        else if (baSurf.GetType() == GeomAbs_SurfaceType::GeomAbs_Cylinder)
        {
            // 获取圆柱面圆柱信息。
        ...
        double & oDist, double oPos_1[3], double oPos_2[3], int iModelId_1, int iModelId_2)
    {
        ...
        try
        {
            ...
            if (instance1)
            {
                ...
                if (transTool)
                {
                    const gp_Trsf* transf = transTool->getInternalTransform();
                    if (transf)
                    {
        ...
        try
        {
            ...
            for (int i = 0; i < vFaceMgr->getDataCount(); i++)
            {
                ...
                for (iter.Init(face, TopAbs_ShapeEnum::TopAbs_WIRE); iter.More(); iter.Next())
                {
                    ...
                    bool flag = anaWire.CheckClosed(1e-3); // Precision::Confusion()
                    if (flag)
                    {
                        Interface::VirtualShape vShape;
                        vShape.CmdId = cmdId;
    ...
    bool FITKOCCCommonTool::getSlice(const int cmdId, double* iOrg, double* iNor, QHash<int, Interface::FITKGeoMeshVSFaceEntity*>& oSolidFaces,
        int iModelId)
    {
        Interface::FITKAbsGeoCommand* cmd = FITKDATAREPO->getTDataByID<Interface::FITKAbsGeoCommand>(cmdId);
        if (!cmd)
        ...
        // 创建无限大的平面。
        gp_Pnt org(iOrg[0], iOrg[1], iOrg[2]);
        gp_Vec nor(iNor[0], iNor[1], iNor[2]);
        nor.Normalize();
        if (qAbs(nor.Magnitude()) < Precision::Confusion())
        {
            return false;
        }

        gp_Pln pln(org, nor);
        BRepBuilderAPI_MakeFace mkFace(pln);

        // 尝试获取模型变换。
        const gp_Trsf* transf{ nullptr };
        Interface::FITKAbstractAssInstance* instance = FITKDATAREPO->getTDataByID<Interface::FITKAbstractAssInstance>(iModelId);
        if (instance)
        {
            FITKOCCTransformTool* transTool = dynamic_cast<FITKOCCTransformTool*>(instance->getInternalTransformTool());
            if (transTool)
            {
                transf = transTool->getInternalTransform();
            }
        }

        int nSolids = vSolidMgr->getDataCount();
        for (int i = 0; i < nSolids; i++)
        {
            Interface::FITKAbsVirtualTopo* vSolid = vSolidMgr->getDataByIndex(i);
            if (!vSolid)
            {
                continue;
            }

            FITKOCCTopoShape* topoShape = vSolid->getShapeT<FITKOCCTopoShape>();
            if (!topoShape)
            {
                continue;
            }

            const TopoDS_Shape& occShape = topoShape->getTopoShape();
            TopoDS_Shape solidShape = occShape;

            // 模型变换。
            if (transf)
            {
                TopoDS_Shape tranShape = BRepBuilderAPI_Transform(occShape, *transf, true);
                if (!tranShape.IsNull())
                {
                    solidShape = tranShape;
                }
            }

            BRepAlgoAPI_Section secLine(mkFace, solidShape);
            secLine.Build();

            if (secLine.IsDone() && !FITKOCCExtendTool::IsEmptyShape(secLine.Shape()))
            {
                BRepAlgoAPI_Common intSec(mkFace, solidShape);
                if (intSec.IsDone() && !intSec.Shape().IsNull())
                {
                    ...
                    int iPtStart = 0;
                    TopExp_Explorer iter;
                    for (iter.Init(intSec, TopAbs_ShapeEnum::TopAbs_FACE); iter.More(); iter.Next())
                    {
                        ...
                        double lineDef = 0.001 * sizeFactor;
                        ...
                        // 网格划分。（三角面片化）
                        BRepMesh_IncrementalMesh(face, params);
                        ...
                        for (Standard_Integer i = 1; i <= nCells; i++)
                        {
                            ...
                            {
                                Interface::FITKGeoMeshVSTri* vsTri = new Interface::FITKGeoMeshVSTri(
                                    pt3 - 1 + iPtStart, pt2 - 1 + iPtStart, pt1 - 1 + iPtStart);
                                vsface->appendMeshTri(vsTri);
                            }
                            else
                            {
                                Interface::FITKGeoMeshVSTri* vsTri = new Interface::FITKGeoMeshVSTri(
                                    pt1 - 1 + iPtStart, pt2 - 1 + iPtStart, pt3 - 1 + iPtStart);
                                vsface->appendMeshTri(vsTri);
                            }
                        }
#else       
                    ...
                    if (vsface->getTriangleCount() == 0)
                    {
                        delete vsface;
                    }
                    else
                    {
                        oSolidFaces.insert(vSolid->getDataObjectID(), vsface);
                    }
                }
            }
        }
    ...
    bool FITKOCCCommonTool::getWireSegs(QList<QList<double>>& iPoses, QList<QList<double>>& oSegPoses, bool isSpline)
    {
        oSegPoses.clear();

        const int nPoses = iPoses.size();
        if (nPoses < 2)
        {
            return false;
        }

        for (const QList<double>& pos : iPoses)
        {
            if (pos.size() < 3)
            {
                return false;
        ...
        auto appendSegPairs = [&oSegPoses](const QList<QList<double>>& pts)
        {
            for (int i = 1; i < pts.size(); ++i)
            {
                oSegPoses.append(pts.at(i - 1));
                oSegPoses.append(pts.at(i));
            }
        };

        // 多段线按线段对返回：1-2, 3-4, 5-6 ...
        if (!isSpline)
        {
            appendSegPairs(iPoses);
            return !oSegPoses.isEmpty();
        }

        // 样条：使用输入坐标作为控制点构造曲线并离散化。
        if (nPoses == 2)
        {
            appendSegPairs(iPoses);
            return !oSegPoses.isEmpty();
        ...
        Handle(TColgp_HArray1OfPnt) poles = new TColgp_HArray1OfPnt(1, nPoses);
        for (int i = 0; i < nPoses; ++i)
        {
            const QList<double>& pos = iPoses.at(i);
            poles->SetValue(i + 1, gp_Pnt(pos.at(0), pos.at(1), pos.at(2)));
        }

        GeomAPI_Interpolate splineBuilder(poles, false, 1e-7);
        splineBuilder.Perform();
        ...
        const double first = adaptorCurve.FirstParameter();
        const double last = adaptorCurve.LastParameter();
        if (fabs(last - first) < Precision::Confusion())
        {
            return false;
        }

        QList<QList<double>> sampledPoses;
        const int sampleCount = (nPoses - 1) * 20 + 1;
        for (int i = 0; i < sampleCount; ++i)
        {
            const double u = first + (last - first) * i / (sampleCount - 1);
            gp_Pnt p;
            spline->D0(u, p);
            sampledPoses.append({ p.X(), p.Y(), p.Z() });
        }

        appendSegPairs(sampledPoses);
