...
namespace OCC
{
    ...
    bool FITKOCCModelCircularHole::update()
    {
        ...
        double depth = sqrt(pow(max3[0] - min3[0], 2) + pow(max3[1] - min3[1], 2) + pow(max3[2] - min3[2], 2)) * 3;
        ...
        // 异常检查。
        if (curDir1.IsParallel(curDir2, Precision::Angular()))
        {
            return false;
        }

        // 获取两直线交点。
        GeomAPI_ExtremaCurveCurve extLinLin(cur1, cur2);
        if (extLinLin.NbExtrema() == 0)
        {
            return false;
        }

        gp_Dir dir = surfNor.Reversed();

        if (topoFace.Orientation() == TopAbs_Orientation::TopAbs_REVERSED)
        {
            dir.Reverse();
        }
        ...
        gp_Pnt cpt1, cpt2;
        extLinLin.Points(1, cpt1, cpt2);

        // 获取所选平面OBB盒子中心点。
        Bnd_OBB obbBox;
        BRepBndLib::AddOBB(topoFace, obbBox);
        const gp_XYZ & obbCt = obbBox.Center();

        // 获取所选平面中心在平面的投影位置。
        GeomAPI_ProjectPointOnSurf projSurf;
        // projSurf.Init(gp_Pnt(cent3[0], cent3[1], cent3[2]), surf);
        projSurf.Init(obbCt, surf);
        ...
        // !!!互相!!!取交点到投影点为平移方向。
        gp_Dir curNPDir2(ctOnCur1.Coord(1) - cpt1.Coord(1), ctOnCur1.Coord(2) - cpt1.Coord(2),
            ctOnCur1.Coord(3) - cpt1.Coord(3));
        gp_Dir curNPDir1(ctOnCur2.Coord(1) - cpt2.Coord(1), ctOnCur2.Coord(2) - cpt2.Coord(2),
            ctOnCur2.Coord(3) - cpt2.Coord(3));

        //// 获取直线垂直方向。
        //gp_Dir curNPDir1 = curDir1.Crossed(surfNor);
        //gp_Dir curNPDir2 = curDir2.Crossed(surfNor);

        //// 根据直线法向与线上任意一点与所选平面OBB盒子中心点投影到面上位置的连线夹角判断距离取向策略。
        ...
        //pt1.SetCoord(1, pt1.Coord(1) + surfNor.Coord(1) * depth * 0.5);
        //pt1.SetCoord(2, pt1.Coord(2) + surfNor.Coord(2) * depth * 0.5);
        //pt1.SetCoord(3, pt1.Coord(3) + surfNor.Coord(3) * depth * 0.5);
        ...
        try
        {
            ...
            cylAxis.SetDirection(dir);

            const TopoDS_Shape & cylinderShape = BRepPrimAPI_MakeCylinder(cylAxis, _diameter * 0.5, depth);
