...
namespace OCC
{
    FITKOCCModelShellFromSolid::FITKOCCModelShellFromSolid() : OCCShapeAgent(this)
    {
        _shapeAgent = _occShapeAgent;
    }

    bool FITKOCCModelShellFromSolid::update()
    {
        QList<Interface::VirtualShape> sourceSolids = solids();
        if (sourceSolids.isEmpty())
        {
            return false;
        }
        ...
        try
        {
            ...
            for (int i = 0; i < sourceSolids.size(); i++)
            {
                Interface::VirtualShape vSolid = sourceSolids[i];
                Interface::FITKAbsGeoCommand* solidCmd = cmdList->getDataByID(vSolid.CmdId);
                if (!solidCmd)
                {
                    return false;
                }

                FITKOCCTopoShape* solidTopo = solidCmd->getShapeTopoByIndexT<FITKOCCTopoShape>(
                    Interface::FITKGeoEnum::VTopoShapeType::VSSolid, vSolid.VirtualTopoIndex);
                if (!solidTopo)
                {
                    return false;
                }

                const TopoDS_Shape & solidShape = solidTopo->getTopoShape();
                if (solidShape.IsNull() || solidShape.ShapeType() != TopAbs_SOLID)
                {
                    return false;
                }

                BRep_Builder builder;
                TopoDS_Compound shellCompound;
                builder.MakeCompound(shellCompound);

                int shellCount = 0;
                for (TopExp_Explorer shellExp(solidShape, TopAbs_SHELL); shellExp.More(); shellExp.Next())
                {
                    TopoDS_Shape shellShape = BRepBuilderAPI_Copy(shellExp.Current()).Shape();
                    if (shellShape.IsNull())
                    {
                        return false;
                    }

                    builder.Add(shellCompound, shellShape);
                    shellCount++;
                }
                ...
                reShape->Replace(solidShape, shellCompound);
            }

            TopoDS_Shape resultShape = reShape->Apply(*partShape);
    ...
    bool FITKOCCModelClosedWireSurface::update()
    {
        ...
        for (int i = 0; i < m_Edges.size(); ++i) {
            ...
            auto vshape = cmd->getShapeTopoByIndexT<FITKOCCTopoShape>(type, edge.VirtualTopoIndex);
        ...
        auto vshape = cmd->getShapeTopoByIndexT<FITKOCCTopoShape>(Interface::FITKGeoEnum::VTopoShapeType::VSEdge, edge.VirtualTopoIndex);
        ...
        if (cmd == nullptr) return nullptr;
        vshape = cmd->getShapeTopoByIndexT<FITKOCCTopoShape>(Interface::FITKGeoEnum::VTopoShapeType::VSEdge, edge.VirtualTopoIndex);
        if (vshape == nullptr) return nullptr;
    ...
    bool FITKOCCModelCoverEdges::update()
    {
        ...
        if (part && part->getAutoBool())
        {
            ...
            TopoDS_Shape result;
            bool flag = FITKOCCComandCommon::MergeShapes(*partShape, topoShape, result, !part->getAutoMergeSolid());
            if (!flag)
            {
                return false;
            }
    ...
    bool FITKOCCModelOffsetSurface::update()
    {
        ...
        if (cmd == nullptr) return false;
        auto vshape = cmd->getShapeTopoByIndexT<FITKOCCTopoShape>(Interface::FITKGeoEnum::VTopoShapeType::VSFace, m_SourceSurface.VirtualTopoIndex);
        if (vshape == nullptr) return false;
        auto topoShape = vshape->getTopoShape();
    ...
    bool FITKOCCModelExtrudeSurface::update()
    {
        ...
        try
        {
            if (m_ExtrudeType == 1)
            {
                ...
                {
                    auto shape = cmd->getShapeTopoByIndexT<FITKOCCTopoShape>(Interface::FITKGeoEnum::VTopoShapeType::VSEdge, m_SourceCurve.VirtualTopoIndex);
    ...
    bool FITKOCCModelRevolSurface::update()
    {
        ...
        else
        {
            ...
            const TopoDS_Edge & e = TopoDS::Edge(shape->getTopoShape());
            if (e.IsNull())
            {
                return false;
            }
            ...
            try
            {
                topoShape = BRepPrimAPI_MakeRevol(mkWire, ax1, m_Angle  * M_PI / 180);
            }
            catch (...)
            {
                return false;
    ...
    bool FITKOCCModelSweepSurface::update()
    {
        ...
        try
        {
            ...
            {
                auto shape = profileCmd->getShapeTopoByIndexT<FITKOCCTopoShape>(Interface::FITKGeoEnum::VTopoShapeType::VSEdge, m_Profiles.at(0).VirtualTopoIndex);
                if (shape == nullptr) return false;

                QList<TopoDS_Wire> wiresProf;
                FITKOCCComandCommon::MakeWires(shape->getTopoShape(), wiresProf, 1e-5);
                if (wiresProf.count() == 0)
                {
                    return false;
                }

                //TopoDS_Compound compProf;
                //builder.MakeCompound(compProf);
                //for (const TopoDS_Wire& w : wiresProf)
                //{
                //    builder.Add(compProf, w);
                //}
                ...
                for (const TopoDS_Wire& w : wiresProf)
                {
                    ...
                    if (FITKOCCExtendTool::IsEmptyShape(retShape))
                    {
                        retShape = mp.Shape();
                    }
                    else
                    {
                        BRepAlgoAPI_Fuse add = BRepAlgoAPI_Fuse(retShape, mp.Shape());
                        if (!add.IsDone())
                        {
                            return false;
                        }
        ...
        if (FITKOCCExtendTool::IsEmptyShape(topoShape))
        {
            return false;
        }
    ...
    bool FITKOCCModelMultiSectionSurface::update()
    {
        ...
        for (auto section : m_Sections) 
        {
            ...
            if (cmd->getGeometryCommandType() == Interface::FITKGeoEnum::FITKGeometryComType::FGTSketch2D)
            {
                ...
                topoShapeSec = BRepBuilderAPI_Copy(*sketchShape);
            }
            // 几何边。
            else
            {
                auto shape = cmd->getShapeTopoByIndexT<FITKOCCTopoShape>(Interface::FITKGeoEnum::VTopoShapeType::VSEdge, section.VirtualTopoIndex);
                if (shape == nullptr) return false;
                topoShapeSec = shape->getTopoShape();
            }

            if (topoShapeSec.IsNull())
            {
                return false;
            }

            if (topoShapeSec.ShapeType() == TopAbs_ShapeEnum::TopAbs_EDGE) {
                BRepLib_MakeWire wire(TopoDS::Edge(topoShapeSec));
                thruSection.AddWire(wire.Wire());
            }
            else if (topoShapeSec.ShapeType() == TopAbs_ShapeEnum::TopAbs_WIRE) {
                thruSection.AddWire(TopoDS::Wire(topoShapeSec));
            }
            else if (topoShapeSec.ShapeType() == TopAbs_ShapeEnum::TopAbs_COMPOUND) {
                TopExp_Explorer exp;
                BRepLib_MakeWire wire;
                // 按封闭曲线分组
                for (exp.Init(topoShapeSec, TopAbs_EDGE); exp.More(); exp.Next())
                {
                    TopoDS_Edge edge = TopoDS::Edge(exp.Current());
                    if (edge.IsNull()) continue;
                    wire.Add(edge);
    ...
    bool FITKOCCModelBridgeSurface::update()
    {
        ...
        auto faceVshape1 = surfaceCmd1->getShapeTopoByIndexT<FITKOCCTopoShape>(Interface::FITKGeoEnum::VTopoShapeType::VSFace, m_SourceSurface1.VirtualTopoIndex);
        ...
        auto faceVshape2 = surfaceCmd2->getShapeTopoByIndexT<FITKOCCTopoShape>(Interface::FITKGeoEnum::VTopoShapeType::VSFace, m_SourceSurface2.VirtualTopoIndex);
