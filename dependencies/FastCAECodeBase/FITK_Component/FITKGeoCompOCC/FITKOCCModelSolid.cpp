...
namespace OCC
{
     ...
     * @brief       将壳面转换为实体。
     * @param[in]   shell：源壳面
     * @param[out]  solidShape：实体形状
     * @return      是否成功
     * @author      ChengHaotian (yeguangbaozi@foxmail.com)
     * @date        2026-07-06
     */
    static bool MakeSolidFromShell(const TopoDS_Shell & shell, TopoDS_Shape & solidShape)
    {
        if (shell.IsNull())
        {
            return false;
        ...
        TopoDS_Shell fixedShell = shell;
        if (!shellFixer.Shell().IsNull())
        {
        ...
        BRepCheck_Analyzer checker(solid);
        if (!checker.IsValid())
        {
            return false;
        }
    ...
    FITKOCCModelSolidFromShell::FITKOCCModelSolidFromShell() : OCCShapeAgent(this)
    {
        _shapeAgent = _occShapeAgent;
    }

    bool FITKOCCModelSolidFromShell::update()
    {
        QList<Interface::VirtualShape> sourceFaces = faces();
        ...
        QList<Interface::FITKAbsVirtualTopo*> commonShells;

        try
        {
            for (int i = 0; i < sourceFaces.size(); i++)
            {
                Interface::VirtualShape iFace = sourceFaces[i];
                Interface::FITKAbsGeoCommand* faceCmd = cmdList->getDataByID(iFace.CmdId);
                if (!faceCmd)
                {
                    return false;
                }

                Interface::FITKAbsVirtualTopo* vFace = faceCmd->getVirtualTopoByIndex(Interface::FITKGeoEnum::VTopoShapeType::VSFace, iFace.VirtualTopoIndex);
                if (!vFace)
                {
                    return false;
                }
                
                FITKOCCTopoShape* faceTopo = vFace->getShapeT<FITKOCCTopoShape>();
                if (!faceTopo)
                {
                    return false;
                }
                ...
                mapper.mapTopo(vFace, Interface::FITKGeoEnum::VSShell, true);
                int shellCount = mapper.length();
                if (shellCount == 0)
                {
                    return false;
                }

                QList<Interface::FITKAbsVirtualTopo*> faceShells;
                for (int j = 0; j < shellCount; j++)
                {
                    Interface::FITKAbsVirtualTopo* vShell = mapper.virtualTopo(j);
                    if (vShell != nullptr && !faceShells.contains(vShell))
                    {
                        faceShells.append(vShell);
                    }
                }

                if (faceShells.isEmpty())
                {
                    return false;
                }
                ...
                if (i == 0)
                {
                    commonShells = faceShells;
                }
                else
                {
                    for (int j = commonShells.size() - 1; j >= 0; j--)
                    {
                        if (!faceShells.contains(commonShells[j]))
                        {
                            commonShells.removeAt(j);
                        }
                    }
                }

                if (commonShells.isEmpty())
                {
                    return false;
                }
            }

            for (int i = 0; i < commonShells.size(); i++)
            {
                Interface::FITKAbsVirtualTopo* vShell = commonShells[i];
                if (vShell == nullptr)
                {
                    return false;
                }

                Interface::FITKVirtualTopoMapper solidMapper;
                solidMapper.mapTopo(vShell, Interface::FITKGeoEnum::VSSolid, true);
                if (solidMapper.length() > 0)
                {
                    return false;
                }
            }
            ...
            for (int i = 0; i < commonShells.size(); i++)
            {
                ...
                FITKOCCTopoShape* occShell = vShell->getShapeT<FITKOCCTopoShape>();
                if (occShell == nullptr)
                {
                    return false;
                }

                TopoDS_Shape shellShape = occShell->getTopoShape();
                ...
                for (TopExp_Explorer faceExp(shellShape, TopAbs_FACE); faceExp.More(); faceExp.Next())
                {
                    sewer.Add(faceExp.Current());
                }
                sewer.Perform();
                ...
                for (TopExp_Explorer shellExp(sewedShape, TopAbs_SHELL); shellExp.More(); shellExp.Next())
                {
                    ...
                    if (!MakeSolidFromShell(shell, solidShape))
                    {
                        return false;
                    }

                    builder.Add(solidCompound, solidShape);
                    currentSolidCount++;
                    solidCount++;
                }
                ...
                reShape->Replace(shellShape, solidCompound);
            }
    ...
    bool FITKOCCModelClosedSurfaceSolid::update()
    {
        ...
        for (int i = 0; i < m_Faces.size(); ++i) {
            ...
            auto vshape = cmd->getShapeTopoByIndexT<FITKOCCTopoShape>(Interface::FITKGeoEnum::VTopoShapeType::VSFace, face.VirtualTopoIndex);
            if (vshape == nullptr)
            {
    ...
    bool FITKOCCModelExtrudeSolid::update()
    {
        ...
        // 处理结果实体，去除异常旋转产生的壳面。
        QList<TopoDS_Shape> retSubShapes, retSubShapes2D, retSubShapes3D;
        FITKOCCComandCommon::FlattenCompShape2D3D(topoShape, retSubShapes, retSubShapes2D, retSubShapes3D);

        if (retSubShapes3D.isEmpty())
        {
            return false;
        }

        // 将全部实体布尔求和进行合并。
        topoShape = retSubShapes3D.takeFirst();

        for (const TopoDS_Shape& s : retSubShapes3D)
        {
            if (s.IsNull())
            {
                continue;
            ...
            BRepAlgoAPI_Fuse merger(topoShape, s);
            if (merger.IsDone())
            {
                topoShape = merger.Shape();
            }
        }
    ...
    bool FITKOCCModelSweepSolid::update()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                //TopoDS_Face face = TopoDS::Face(BRepBuilderAPI_Copy(faceShape));
    ...
    bool FITKOCCModelMultiSectionSolid::update()
    {
        ...
        for (auto section : m_Sections) {
            ...
            {
                auto shape = cmd->getShapeTopoByIndexT<FITKOCCTopoShape>(Interface::FITKGeoEnum::VTopoShapeType::VSFace, section.VirtualTopoIndex);
                if (shape == nullptr) return false;
