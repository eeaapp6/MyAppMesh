...
#include "FITKOCCViewAdaptorSketchCommand.h"
...
namespace Exchange
{
    bool FITKOCCViewAdaptorSketchCommand::update()
    {
        if (!_dataObj)
        {
            return false;
        ...
        Interface::FITKAbsGeoSketch2D* sketch = dynamic_cast<Interface::FITKAbsGeoSketch2D*>(_dataObj);
        if (!sketch)
        {
            return false;
        }
        ...
        FITKOCCGraphObjectSketchCommand* outputData = new FITKOCCGraphObjectSketchCommand(sketch);
