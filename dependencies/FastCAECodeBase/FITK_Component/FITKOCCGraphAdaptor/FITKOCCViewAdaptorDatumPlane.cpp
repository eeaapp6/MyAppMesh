...
#include "FITKOCCViewAdaptorDatumPlane.h"
...
namespace Exchange
{
    bool FITKOCCViewAdaptorDatumPlane::update()
    {
        if (!_dataObj)
        {
            return false;
        ...
        Interface::FITKAbsGeoDatumPlane* datum = dynamic_cast<Interface::FITKAbsGeoDatumPlane*>(_dataObj);
        if (!datum)
        {
            return false;
        }
        ...
        FITKOCCGraphObjectDatumPlane* outputData = new FITKOCCGraphObjectDatumPlane(datum);
