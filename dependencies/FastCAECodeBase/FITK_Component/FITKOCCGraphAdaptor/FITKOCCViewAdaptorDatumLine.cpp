...
#include "FITKOCCViewAdaptorDatumLine.h"
...
namespace Exchange
{
    bool FITKOCCViewAdaptorDatumLine::update()
    {
        if (!_dataObj)
        {
            return false;
        ...
        Interface::FITKAbsGeoDatumLine* datum = dynamic_cast<Interface::FITKAbsGeoDatumLine*>(_dataObj);
        if (!datum)
        {
            return false;
        }
        ...
        FITKOCCGraphObjectDatumLine* outputData = new FITKOCCGraphObjectDatumLine(datum);
