...
namespace Interface
{
    ...
    bool FITKModelSurface::isValid()
    {
        ...
        for (int i = 0 ; i < _modelSetIDs.size(); i++)
        {
            ...
            valid &= model->isComponentValid(set->getModelSetType(), set->getAbsoluteMember());
        }

        return valid;
    }

    bool FITKModelSurface::isEmpty()
    {
        bool empty = true;
        ...
        for (int i = 0; i < _modelSetIDs.size(); i++)
        {
            ...
            empty &= set->getAbsoluteMember().isEmpty();
        }
