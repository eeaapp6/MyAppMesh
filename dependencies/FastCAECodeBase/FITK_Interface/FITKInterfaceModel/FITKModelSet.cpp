...
namespace Interface
{
    ...
    bool FITKModelSet::isValid()
    {
        ...
        if (nSubSets == 0)
        {
            valid = model->isComponentValid(this->getModelSetType(), this->getAbsoluteMember());
        }
        else
        {
    ...
    bool FITKModelSet::isEmpty()
    {
        bool empty = true;
        int nSubSets = getDataCount();
        if (nSubSets == 0)
        {
            empty = this->getAbsoluteMember().isEmpty();
        }
        else
        {
            for (int i = 0; i < nSubSets; i++)
