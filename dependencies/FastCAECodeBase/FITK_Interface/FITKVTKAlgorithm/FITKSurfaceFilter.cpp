...
QList<int> FITKSurfaceFilter::getOuterSurfaceCellIds(int originCellId)
{
    if (originCellId < 0 || originCellId >= _outerSurfaceCellFlags.size())
    {
        return QList<int>();
    }

    const std::vector<bool> & surfaceFlags = _outerSurfaceCellFlags[originCellId];
    QList<int> outerSurfaceCellIds;
    for (int i = 0; i < surfaceFlags.size(); ++i)
    {
        if (surfaceFlags[i])
        {
            const int idx = getSurfaceCellIdByOriginCellId(originCellId, i);
            if (idx >= 0)
            {
                outerSurfaceCellIds.append(idx);
            }
        }
    }

    return outerSurfaceCellIds;
}

bool FITKSurfaceFilter::isOuterSurf(int orgCellId, int surfaceIndex)
{
    if (orgCellId < 0 || surfaceIndex < 0 || orgCellId >= _outerSurfaceCellFlags.size())
    {
        return false;
    }

    const std::vector<bool> & surfaceFlags = _outerSurfaceCellFlags[orgCellId];
    if (surfaceIndex >= surfaceFlags.size())
    {
        return false;
    }

    return surfaceFlags[surfaceIndex];
}
...
FITKSurfaceFilter::FITKSurfaceFilter()
{
    ...
    this->CellFaceIdx = nullptr;
    this->OriginalCellIdsName = nullptr;
    this->OriginalPointIdsName = nullptr;
    this->CellFaceIdxName = nullptr;

    this->NonlinearSubdivisionLevel = 0;
}
...
FITKSurfaceFilter::~FITKSurfaceFilter()
{
    ...
    this->SetCellFaceIdxName(nullptr);
}
    ...
    vtkInformationVector *outputVector)
{
    ...
    // Initialize surface cell flags.
    int ncs = input->GetNumberOfCells();
    _outerSurfaceCellFlags.resize(ncs);

    for (int i = 0; i < ncs; i++)
    {
        _outerSurfaceCellFlags[i].resize(std::max(1, input->GetCell(i)->GetNumberOfFaces()));
        std::fill(_outerSurfaceCellFlags[i].begin(), _outerSurfaceCellFlags[i].end(), false);
    }
    ...
    case VTK_POLY_DATA:
    {
        ...
        this->CellFaceIdx = vtkIntArray::New();
        this->CellFaceIdx->SetName(this->GetCellFaceIdxName());
        this->CellFaceIdx->SetNumberOfComponents(1);

        vtkIdType numTup = output->GetNumberOfCells();
        this->OriginalCellDims->SetNumberOfValues(numTup);
        this->CellFaceIdx->SetNumberOfValues(numTup);
        for (vtkIdType cId = 0; cId < numTup; cId++)
        {
            vtkCell* ic = output->GetCell(cId);
            this->OriginalCellDims->SetValue(cId, ic->GetCellDimension());
            this->CellFaceIdx->SetValue(cId, 0);
        }

        output->GetCellData()->AddArray(this->OriginalCellDims);
        output->GetCellData()->AddArray(this->CellFaceIdx);

        this->OriginalCellDims->Delete();
        this->OriginalCellDims = nullptr;
        this->CellFaceIdx->Delete();
        this->CellFaceIdx = nullptr;
    ...
    vtkPolyData *output)
{
    ...
    this->CellFaceIdx->SetName(this->GetCellFaceIdxName());
    this->CellFaceIdx->SetNumberOfComponents(1);
    outputCD->AddArray(this->CellFaceIdx);
...
//    }
                        ...
                        if (cellIds->GetNumberOfIds() <= 0)
                        {
                            ...
                            else
                            {
                                ...
                                case VTK_QUADRATIC_TRIANGLE:
                                {
                                    ...
                                    vtkIdType idsPoly[6]{ qcids[5], qcids[0], qcids[3], qcids[1], qcids[4], qcids[2] };             
                                    this->InsertPolygonInHash(idsPoly, 6, cellId, 3);

                                    //vtkIdType ids[3]{ qcids[0], qcids[1], qcids[2]  };
                                    //this->InsertPolygonInHash(ids, 3, cellId, 3);

                                    /*pointIdList = face->PointIds;
                                    ids = pointIdList->GetPointer(0);
                                    pts->Reset();

                                    vtkIdType idsPoly[6]{ ids[5], ids[0], ids[3], ids[1], ids[4], ids[2] };
                                    for (i = 0; i < 6; i++)
                                    {
                                        inPtId = idsPoly[i];
                                        outPtId = this->GetOutputPointId(inPtId, input, newPts, outputPD);
                                ...
                                {
                                    vtkIdType* qcids = face->PointIds->GetPointer(0);
                                    vtkIdType ids[8]{ qcids[7], qcids[0], qcids[4], qcids[1], qcids[5], qcids[2], qcids[6], qcids[3] };
                                    this->InsertPolygonInHash(ids, 8, cellId, 3);
                                    //vtkIdType ids[4]{ qcids[0], qcids[1], qcids[2], qcids[3] };
                                    //this->InsertPolygonInHash(ids, 4, cellId, 3);
                                    break;
                                }
#if VTK_MAJOR_VERSION >= 9
                                case VTK_LAGRANGE_QUADRILATERAL:
        ...
        cellIter->GoToNextCell())
    {
            ...
            || cellType == VTK_QUADRATIC_LINEAR_QUAD)
        {
            ...
            vtkIdType idsPoly[8]{ ids[7], ids[0], ids[4], ids[1], ids[5], ids[2], ids[6], ids[3] };
            for (i = 0; i < 8; i++)
            {
                inPtId = idsPoly[i];
                outPtId = this->GetOutputPointId(inPtId, input, newPts, outputPD);
    ...
    vtkIdType oriSurIndex)
{
    ...
    _outerSurfaceCellFlags[originalId][oriSurIndex] = true;

    if (this->CellFaceIdx)
    {
        this->CellFaceIdx->InsertValue(destIndex, oriSurIndex);
    }
}
