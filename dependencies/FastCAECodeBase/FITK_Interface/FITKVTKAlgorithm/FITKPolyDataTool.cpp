...
namespace Common
{
    ...
    QList<int> FITKPolyDataTool::getCoPlanePointIDs(int seedID, double angleHold)
    {
        ...
        for (const int & cell : neighborCells)
        {
            cellPointIds->Reset();
            cellPointIds->Resize(0);
            _poly->GetCellPoints(cell, cellPointIds);

            int nPts = cellPointIds->GetNumberOfIds();
            vtkIdType* pIds = cellPointIds->GetPointer(0);

            for (vtkIdType i = 0; i < nPts; i++)
            {
                // Modified by CHT
                //@{
                m_scanPointCoPlaneFlag->SetValue(pIds[i], 1);
        ...
        int* pFlag = m_scanPointCoPlaneFlag->GetPointer(0);
        for (int i = 0; i < nPt; i++)
        {
            if (pFlag[i])
            {
    ...
    {
        if (_poly == nullptr || _normals == nullptr || seedID < 0 || seedID >= _poly->GetNumberOfCells()) return QList<int>();

        const double cosHold = cos(angleHold * 3.141592654 / 180.0);
        QList<int> coPlaneIDs;
        QList<int> afWave;
        coPlaneIDs.append(seedID);
        afWave.append(seedID);
        m_scanCellCoPlaneFlag->SetValue(seedID, 1);

        while (!afWave.isEmpty())
        {
            const int cell0 = afWave.takeFirst();
            double normal0[3] = { 0, 0, 0 };
            _normals->GetTuple(cell0, normal0);
            const double norm0 = vtkMath::Norm(normal0);
            if (norm0 <= 1e-7)
            {
                continue;
            }

            const QList<CELLPAIR> neighborCells = getNeighborCell(cell0/*, scanedCell*/);

            double normal1[3] = { 0, 0, 0 };
            for (const CELLPAIR & p : neighborCells)
            {
                const int cell1 = p.first;
                _normals->GetTuple(cell1, normal1);
                const double norm1 = vtkMath::Norm(normal1);
                if (norm1 <= 1e-7)
                {
                    continue;
                }

                double value = vtkMath::Dot(normal0, normal1) / (norm0 * norm1);
                value = value > 0.99999999999999 ? 0.99999999999999 : value;
                value = value < -0.99999999999999 ? -0.99999999999999 : value;

                if (value >= cosHold)
                {
                    if (!m_scanCellCoPlaneFlag->GetValue(cell1))
                    {
                        afWave.append(cell1);
                        coPlaneIDs.append(cell1);
    ...
    //        //计算所有最外层单元的邻接单元
    //        for (const int & cellID : tempAF)
    //        {
    //            QList<CELLPAIR> pa = getNeighborCell(cellID/*, scanedCell*/);
    //            afRound.append(pa);
    //        }
    //        //计算法向夹角，产生新的最外层单元
    //        for (const CELLPAIR & p : afRound)
    //        {
    ...
    QList<int> FITKPolyDataTool::getEdgePointsByAngle(int pointID, double angle)
    {
        ...
        for (int iCell = 0; iCell < iCellNum; ++iCell)
        {
            ...
            for (const int & findCellID : lines)
            {
                vtkCell* findCell = _poly->GetCell(findCellID);
                if (!findCell) { return result.toList(); }
    ...
    int FITKPolyDataTool::getNextEdgeLineByAngle(int cellID, int pointIndex, double angle)
    {
        ...
        {
            int findCellID = cellIDPtr[iCell];
    ...
    QList<int> FITKPolyDataTool::getEdgeLinesByDirectionAndAngle(int cellID, double angle, bool endWithCross /*= false */)
    {
        ...
        vtkCell* seedCell = _poly->GetCell(cellID);
        QList<QPair<int, int>> lookupCellIDs;
        lookupCellIDs.append(QPair<int, int>(cellID, seedCell->GetPointId(1)));
        lookupCellIDs.append(QPair<int, int>(cellID, seedCell->GetPointId(0)));
        finishedCellIDs.append(cellID);

        while (!lookupCellIDs.isEmpty())
        {
            QPair<int, int> current = lookupCellIDs.takeFirst();
            int currentCellID = current.first;
            int lookupPointID = current.second;

            double vec1[3] = { 0 };
            getLineItemVector(currentCellID, vec1);
            double norm1 = vtkMath::Norm(vec1);
            if (norm1 <= 1e-7) { continue; }

            vtkSmartPointer<vtkIdList> cellIDs = vtkSmartPointer<vtkIdList>::New();
            _poly->GetPointCells(lookupPointID, cellIDs);
            int cellsNum = cellIDs->GetNumberOfIds();
            vtkIdType* cellIDPtr = cellIDs->GetPointer(0);

            if (endWithCross && cellsNum > 2) { continue; }

            for (int iCell = 0; iCell < cellsNum; ++iCell)
            {
                int findCellID = cellIDPtr[iCell];
                if (findCellID == currentCellID || finishedCellIDs.contains(findCellID)) { continue; }

                vtkCell* findCell = _poly->GetCell(findCellID);
                int findPt0ID = findCell->GetPointId(0);
                int findPt1ID = findCell->GetPointId(1);
                if (findPt0ID != lookupPointID && findPt1ID != lookupPointID) { continue; }

                double vec2[3] = { .0 };
                getLineItemVector(findCellID, vec2);
                double norm2 = vtkMath::Norm(vec2);
                if (norm2 <= 1e-7) { continue; }

                double cosValue = vtkMath::Dot(vec1, vec2) / (norm1 * norm2);
                cosValue = qMax(-1.0, qMin(1.0, cosValue));
                double currentAngle = vtkMath::DegreesFromRadians(acos(qAbs(cosValue)));
                if (currentAngle >= angle) { continue; }

                int nextPointID = findPt0ID == lookupPointID ? findPt1ID : findPt0ID;
                finishedCellIDs.append(findCellID);
                lookupCellIDs.append(QPair<int, int>(findCellID, nextPointID));
            }
        }
    ...
    QList<int> FITKPolyDataTool::getNextEdgeLineByDirectionAndAngle(int cellID, bool down, double angle, bool endWithCross /*= false */)
    {
        ...
        /// 查找推导方向上，节点所有单元
        vtkCell* cell = _poly->GetCell(cellID);
        int ptID0 = cell->GetPointId(0);
        int ptID1 = cell->GetPointId(1);
        int ptID = down == true ? ptID1 : ptID0;

        if (ptID == cellID && ptID0 == cellID)
        {
            ptID = ptID1;
        }
        else if (ptID == cellID && ptID1 == cellID)
        {
            ptID = ptID0;
        }
        ...
        for (int iCell = 0; iCell < cellsNum; ++iCell)
        {
            ...
            if (findPt0ID != ptID && findPt1ID != ptID) { continue; }
            ...
            double norm1 = vtkMath::Norm(vec1);
            double norm2 = vtkMath::Norm(vec2);
            if (norm1 <= 1e-7 || norm2 <= 1e-7) { continue; }
    ...
    QList<CELLPAIR> FITKPolyDataTool::getNeighborCell(int cellID/*, const QList<int>& scanedCellIDs*/)
    {
        ...
        vtkIdType nPts;
        const vtkIdType* pts;
        _poly->GetCellPoints(cellID, nPts, pts);
        ...
        for (vtkIdType i = 0; i < nPts; i++)
        {
            ...
            neighborCellIds->Resize(0);
            _poly->GetCellNeighbors(cellID, idList, neighborCellIds);

            int nNeis = neighborCellIds->GetNumberOfIds();
            vtkIdType* neighborPts = neighborCellIds->GetPointer(0);

            for (vtkIdType j = 0; j < nNeis; j++)
            {
                const int roundID = neighborPts[j];
