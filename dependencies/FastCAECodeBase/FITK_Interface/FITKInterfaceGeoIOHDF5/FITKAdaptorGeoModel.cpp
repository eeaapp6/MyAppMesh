...
namespace Interface
{
    ...
    bool FITKAdaptorGeoModelFromSketch::readParameters()
    {
        Interface::FITKAbsGeoModelFromSketch* cmd = this->getDataObjectAs<Interface::FITKAbsGeoModelFromSketch>();
        if (cmd == nullptr || _reader == nullptr || _h5Group == nullptr) return false;
        IO::FITKHDF5FileTool* h5tools = _reader->getHDF5FileTool();
        if (h5tools == nullptr) return false;
        ...
        cmd->setSketchDataID(sketchId);
        return true;
    }

    bool FITKAdaptorGeoModelFromSketch::writeParameters(H5::Group &g)
    {
        Interface::FITKAbsGeoModelFromSketch* cmd = this->getDataObjectAs<Interface::FITKAbsGeoModelFromSketch>();
        if (cmd == nullptr || _writer == nullptr || _h5Group == nullptr) return false;
        IO::FITKHDF5FileTool* h5tools = _writer->getHDF5FileTool();
        ...
        std::tuple<bool, H5::Group> paramGroup = h5tools->createGp(g, FGKW_Parameter);
        if (!std::get<0>(paramGroup)) return false;
        h5tools->writeGroupAttrInt(std::get<1>(paramGroup), "sketchId", cmd->getSketchDataID());
        return true;
    }

    bool FITKAdaptorGeoModelPlanarShell::readParameters()
    {
        Interface::FITKAbsGeoModelPlanarShell* cmd = this->getDataObjectAs<Interface::FITKAbsGeoModelPlanarShell>();
        if (cmd == nullptr || _reader == nullptr || _h5Group == nullptr) return false;
        IO::FITKHDF5FileTool* h5tools = _reader->getHDF5FileTool();
        if (h5tools == nullptr) return false;
    ...
    bool FITKAdaptorGeoModelPlanarShell::writeParameters(H5::Group &g)
    {
        Interface::FITKAbsGeoModelPlanarShell* cmd = this->getDataObjectAs<Interface::FITKAbsGeoModelPlanarShell>();
        if (cmd == nullptr || _writer == nullptr || _h5Group == nullptr) return false;
        IO::FITKHDF5FileTool* h5tools = _writer->getHDF5FileTool();
    ...
    bool FITKAdaptorGeoModelShellFromSolid::readParameters()
    {
        Interface::FITKAbsGeoModelShellFromSolid* cmd = this->getDataObjectAs<Interface::FITKAbsGeoModelShellFromSolid>();
        ...
        {
            int solidArray[3] = {};
            if (!h5tools->readGroupAttrIntArray(std::get<1>(paramGroup), std::to_string(i).c_str(), solidArray, 1, 3))
            {
                return false;
            }
            solids.append(Interface::VirtualShape(solidArray[0], solidArray[1], solidArray[2]));
        }
    ...
    bool FITKAdaptorGeoModelShellFromSolid::writeParameters(H5::Group &g)
    {
        Interface::FITKAbsGeoModelShellFromSolid* cmd = this->getDataObjectAs<Interface::FITKAbsGeoModelShellFromSolid>();
        ...
        for (int i = 0; i < count; i++)
        {
            Interface::VirtualShape solid = solids[i];
            int solidArray[3]{ solid.CmdId, solid.VirtualTopoId, solid.VirtualTopoIndex };
            h5tools->writeGroupAttrIntArray(std::get<1>(paramGroup), std::to_string(i).c_str(), solidArray, 1, 3);
        }

        return true;
    }

    bool FITKAdaptorGeoModelWireEdgeFromEdge::readParameters()
    {
    ...
    bool FITKAdaptorGeoModelWireEdgeFromEdge::writeParameters(H5::Group &g)
    {
        Interface::FITKAbsGeoWireEdgeFromEdge* cmd = this->getDataObjectAs<Interface::FITKAbsGeoWireEdgeFromEdge>();
        if (cmd == nullptr || _writer == nullptr || _h5Group == nullptr) return false;
        IO::FITKHDF5FileTool* h5tools = _writer->getHDF5FileTool();
    ...
    bool FITKAdaptorGeoModelRemoveWireEdges::readParameters()
    {
        Interface::FITKAbsGeoRemoveWireEdges* cmd = this->getDataObjectAs<Interface::FITKAbsGeoRemoveWireEdges>();
        if (cmd == nullptr || _reader == nullptr || _h5Group == nullptr) return false;
        IO::FITKHDF5FileTool* h5tools = _reader->getHDF5FileTool();
        if (h5tools == nullptr) return false;
    ...
    bool FITKAdaptorGeoModelRemoveWireEdges::writeParameters(H5::Group &g)
    {
        Interface::FITKAbsGeoRemoveWireEdges* cmd = this->getDataObjectAs<Interface::FITKAbsGeoRemoveWireEdges>();
        if (cmd == nullptr || _writer == nullptr || _h5Group == nullptr) return false;
        IO::FITKHDF5FileTool* h5tools = _writer->getHDF5FileTool();
        ...
        QList<VirtualShape> edges = cmd->getVShapes();
        int count = edges.size();
        h5tools->writeGroupAttrInt(std::get<1>(paramGroup), FGKW_Count, count);
        for (int i = 0; i < count; i++)
        {
