...
#include "FITKAdaptorGeoModelFilledRectangle.h"
...
namespace Interface
{
    bool  FITKAdaptorGeoModelFilledRectangle::readParameters()
    {
        Interface::FITKAbsGeoModelFilledRectangle* cmd = this->getDataObjectAs<Interface::FITKAbsGeoModelFilledRectangle>();
        ...
        if (!h5tools->readGroupDouble(std::get<1>(paramGroup), FGKW_XLength, &xLength)) return false;
        if (!h5tools->readGroupDouble(std::get<1>(paramGroup), FGKW_YLength, &yLength)) return false;
        ...
        cmd->setXLength(xLength);
        cmd->setYLength(yLength);
        cmd->setFilled(isFilled != 0);
        
        return true;
    }
    
    bool FITKAdaptorGeoModelFilledRectangle::writeParameters(H5::Group &g)
    {
        Interface::FITKAbsGeoModelFilledRectangle* cmd = this->getDataObjectAs<Interface::FITKAbsGeoModelFilledRectangle>();
        ...
        h5tools->writeGroupAttrDouble(std::get<1>(paramGroup), FGKW_XLength, cmd->xLength());
        h5tools->writeGroupAttrDouble(std::get<1>(paramGroup), FGKW_YLength, cmd->yLength());
