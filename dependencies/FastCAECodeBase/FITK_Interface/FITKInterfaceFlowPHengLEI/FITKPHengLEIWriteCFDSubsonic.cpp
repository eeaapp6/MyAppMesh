...
#include "FITKPHengLEIWriteCFDSubsonic.h"
...
namespace Interface
{
	bool FITKPHengLEIWriteCFDSubsonic::writeParamFile()
	{
		//创建文件
		...
		Core::FITKParamDouble* dataDouWallTemperature = dynamic_cast<Core::FITKParamDouble*>(dataFlowWallTemperature);
		double valueDataDoudataWallTemperature = dataDouWallTemperature->getValue();
		...
		//获取spatial
		Interface::FITKPHengLEIData* spatialData = Interface::FITKPHengLEIData::GetPHengLEIDataFromFrame();
		if (!spatialData) return false;
		auto getspatialData_ = spatialData->getDiscretisation();
		Interface::FITKPHengLEIAbsSpatialDiscretisation* spatialData_ = getspatialData_->getSpatialDiscretisation();
		//获取空间离散类型
		...
		double valueDataIntCFLVaryStep = dataIntCFLVaryStep->getValue();
		...
		Interface::FITKPHengLEIData* multiGrid = Interface::FITKPHengLEIData::GetPHengLEIDataFromFrame();
		if (!multiGrid) return false;
		FITKPHengLEIMultiGrid* multiGridParam = multiGrid->getMultiGrid();
		...
		*stream << "double wallTemperature = " << valueDataDoudataWallTemperature << ";" << endl;
		...
		*stream << "#######################################" << endl;
		...
		if (spatialType == "UnStruct") 
		{
			...
			*stream << "double venkatCoeff = " << valueDoubleVenkatCoeff << ";" << endl;   //Todo 获取venkatCoeff
			if (Limiter_ == "vencat")
			{
	...
	void FITKPHengLEIWriteCFDSubsonic::consoleMessage(int level, const QString& str)
	{
		return;
	}

	void FITKPHengLEIWriteCFDSubsonic::run()
	{
		writeParamFile();
		closeFileStream();
		return;
