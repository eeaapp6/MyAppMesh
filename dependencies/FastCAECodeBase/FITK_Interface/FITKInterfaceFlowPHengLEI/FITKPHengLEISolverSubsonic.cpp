...
namespace Interface
{
    ...
    FITKPHengLEISubsonicSolver::FITKPHengLEISubsonicSolver()
    {
        this->init();
    }

    FITKPHengLEIAbsSolver::PHengLEISolverType FITKPHengLEISubsonicSolver::getSolverType()
    {
        //亚音速类型
        return  FITKPHengLEIAbsSolver::PHengLEISolverType::PLSubsonic;
    }

    void FITKPHengLEISubsonicSolver::initBoundaryTypes()
    {
    ...
    void FITKPHengLEISubsonicSolver::initBasicInfo()
    {
		if (!_generalControl) return;

		Core::FITKParameter* addData = _generalControl->getGeneralControlAdditionalData();
		if (!addData)return;
		addData->createDoubleParameter("intervalStepFlow", 1000);
		addData->createDoubleParameter("maxSimuStep", 10000);
		addData->createDoubleParameter("intervalStepPlot", 1000);
		addData->createDoubleParameter("intervalStepForce", 100);
		addData->createDoubleParameter("intervalStepRes", 10);
    }

    void FITKPHengLEISubsonicSolver::initInflow()
    {
		if (!_inFlow) return;
		...
		if (!addData) return;
		addData->createDoubleParameter("RefMachNumber", 0.2);
		addData->createDoubleParameter("Attacked", 2.79);
		addData->createDoubleParameter("Angle Slide", 0.00);
		addData->createDoubleParameter("Wall Temperature", -1);

		_inFlow->setInFlowAdditionalData(addData);
    }

    void FITKPHengLEISubsonicSolver::initGridParam()
    {
		if (!_meshPara) return;
    ...
    void FITKPHengLEISubsonicSolver::initViscous()
    {
    ...
    void FITKPHengLEISubsonicSolver::initDiscrete()
    {
			...
			addData->createDoubleParameter("CFL End", 3.0);
    ...
    void FITKPHengLEISubsonicSolver::initMultiGridParam()
    {
	...
	void FITKPHengLEISubsonicSolver::initOutputSetting()
	{
	...
	void FITKPHengLEISubsonicSolver::initFlow()
	{
	...
	void FITKPHengLEISubsonicSolver::initCalculation()
	{
	...
	void FITKPHengLEISubsonicSolver::initInitialCondition()
	{
		if (_initialCondition) delete _initialCondition;
		_initialCondition = nullptr;
	}

	void FITKPHengLEISubsonicSolver::initSolution()
	{
		if (_solution) delete _solution;
		_solution = nullptr;
	}

	void FITKPHengLEISubsonicSolver::initModels()
	{
		// 添加ViscousModel参数
