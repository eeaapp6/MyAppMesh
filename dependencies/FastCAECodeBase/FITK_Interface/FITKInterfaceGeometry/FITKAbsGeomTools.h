...
namespace Interface
{
    ...
    public:
         ...
         * @param[i] edgeVirTopoIDs                     离散边
         * @return   QList<QPair<int,QPair<int,int>>>   排序结果
         * @author   BaGuijun (baguijun@163.com)
         * @date     2025-07-22
         */
        static QList<QPair<int, QPair<int, int>>> sortEdgeInfos(QList<int> edgeVirTopoIDs);
        /**
         * @brief    通过边ID与比例获取点位置
         * @param[i] edgeVirTopoId              边id
         ...
         * @param[in]   iNPoses：采样数量
         * @param[out]  oPoses：返回坐标列表[引用]
         * @param[out]  oDirs：返回切向列表[引用]
         * @param[in]   iModelId：模型ID[缺省]
         * @return      采样结果是否有效
         * @author      ChengHaotian (yeguangbaozi@foxmail.com)
         * @date        2026-03-25
         */
        virtual bool sampleEdgeByID(int vEdgeId, int iNPoses, QVector<QVector<double>> & oPoses, 
            QVector<QVector<double>> & oDirs, int iModelId = -1);
         ...
         * @date        2026-03-25
         */
        virtual bool sampleEdge(FITKAbsVirtualTopo* vtp, int iNPoses, QVector<QVector<double>> & oPoses, 
            QVector<QVector<double>> & oDirs, int iModelId = -1)
        {
            Q_UNUSED(vtp);
            Q_UNUSED(iNPoses);
    ...
    public:
         ...
         * @date        2026-03-24
         */
        virtual bool getCylindSurfInfoByID(const int & id, double* oCt, double* oDir, double & oRadius, double* oBds);
         ...
         * @date        2026-03-24
         */
        virtual bool getCylindSurfInfo(Interface::FITKAbsVirtualTopo* vtp, double* oCt, double* oDir, double & oRadius, double* oBds)
        {
            Q_UNUSED(vtp);
            Q_UNUSED(oCt);
            Q_UNUSED(oDir);
         ...
         * @param[in]   id：虚拓扑ID[引用]
         * @param[out]  oCt：圆锥面中心
         * @param[out]  oApex：圆锥面顶点
         * @param[out]  oDir：圆锥面轴线方向
         * @param[out]  oRadius：圆锥面半径
         * @param[out]  oBds：包围盒
         * @return      是否为圆锥面
         * @author      ChengHaotian (yeguangbaozi@foxmail.com)
         * @date        2026-03-24
         */
        virtual bool getConeSurfInfoByID(const int & id, double* oCt, double* oApex, double* oDir, double & oRadius, double* oBds);
         ...
         * @date        2026-03-24
         */
        virtual bool getConeSurfInfo(FITKAbsVirtualTopo* vtp, double* oCt, double* oApex, double* oDir, double & oRadius, double* oBds)
        {
            Q_UNUSED(vtp);
            Q_UNUSED(oCt);
            Q_UNUSED(oApex);
    ...
    public:
        ...
        /**
         * @brief       获取给定模型的平面切片。[虚函数]
         * @param[in]   cmdId：命令ID
         * @param[in]   iOrg：平面中心
         * @param[in]   iNor：平面法向
         * @param[out]  oSolidFaces：模型切片面三角化数据字典，键为实体ID[引用]
         * @param[in]   iModelId：模型ID[缺省]
         * @return      是否存在有效切片
         * @author      ChengHaotian (yeguangbaozi@foxmail.com)
         * @date        2026-05-25
         */
        virtual bool getSlice(const int cmdId, double* iOrg, double* iNor, 
            QHash<int, FITKGeoMeshVSFaceEntity*> & oSolidFaces,
            int iModelId = -1)
        {
            Q_UNUSED(cmdId);
            Q_UNUSED(iOrg);
            Q_UNUSED(iNor);
            Q_UNUSED(oSolidFaces);
            Q_UNUSED(iModelId);
            return false;
        }
        ...
        /**
         * @brief       获取线边离散化信息。[虚函数]
         * @param[in]   iPoses：输入点坐标
         * @param[out]  oSegPoses：输出线段坐标
         * @param[in]   isSpline：是否为样条曲线[缺省]
         * @return      是否存在有效线段
         * @author      ChengHaotian (yeguangbaozi@foxmail.com)
         * @date        2026-07-02
         */
        virtual bool getWireSegs(QList<QList<double>> & iPoses, 
            QList<QList<double>> & oSegPoses, bool isSpline = false)
        {
            Q_UNUSED(iPoses);
            Q_UNUSED(oSegPoses);
            Q_UNUSED(isSpline);
