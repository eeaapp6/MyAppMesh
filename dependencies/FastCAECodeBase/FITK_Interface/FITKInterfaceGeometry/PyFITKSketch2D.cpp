...
namespace Interface
{
    ...
    bool PyFITKSketch2D::isUsable()
    {
        ...
        if (cmd) {
            //如果该命令用于扫略，则不检查
            int usefor = _sketch2D->userFor();
            if (usefor == FITKAbsGeoSketch2D::SketchUseFor::PartSweepSolidPath ||
                usefor == FITKAbsGeoSketch2D::SketchUseFor::PartSweepSolidProfile ||
                usefor == FITKAbsGeoSketch2D::SketchUseFor::PartSweepShellProfile ||
                usefor == FITKAbsGeoSketch2D::SketchUseFor::PartSweepShellPath ||
                usefor == FITKAbsGeoSketch2D::SketchUseFor::PartSweepCutProfile ||
                usefor == FITKAbsGeoSketch2D::SketchUseFor::PartSweepCutPath)
                return true;
            return false;
        }
        return true;
