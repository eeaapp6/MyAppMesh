...
namespace Interface
{
    ...
    bool FITKAbsGeoShapeAgent::hasTopos()
    {
        int nTopos = _vtmanager->getAllShapeVirtualTopoCount();
        return nTopos > 0;
    }
    ...
    bool FITKAbsGeoShapeAgent::hasModelDim(FITKModelEnum::FITKModelDim dim)
    {
        if (!_vtmanager)
        {
            return false;
        ...
        Interface::FITKGeoEnum::VTopoShapeType pType;
        switch (dim)
        {
        case Interface::FITKModelEnum::FMD0:
            type = Interface::FITKGeoEnum::VSPoint;
            pType = Interface::FITKGeoEnum::VSEdge;
            break;
        case Interface::FITKModelEnum::FMD1:
            type = Interface::FITKGeoEnum::VSEdge;
            pType = Interface::FITKGeoEnum::VSFace;
            break;
        case Interface::FITKModelEnum::FMD2:
            type = Interface::FITKGeoEnum::VSFace;
            pType = Interface::FITKGeoEnum::VSSolid;
            break;
        case Interface::FITKModelEnum::FMD3:
            type = Interface::FITKGeoEnum::VSSolid;
            pType = Interface::FITKGeoEnum::VSSolid;
            break;
        default:
            return false;
        ...
        int nTopos = vTopoMgr->getDataCount();
        if (pType == type)
        {
            return nTopos != 0;
        }
        ...
        // 检查是否存在指定维度的顶层拓扑。
        int iTops = 0;
        FITKVirtualTopoMapper vMapper;
        for (int i = 0; i < nTopos; i++)
        {
            Interface::FITKAbsVirtualTopo* vTopo = vTopoMgr->getDataByIndex(i);
            if (!vTopo)
            {
                continue;
            }

            vMapper.mapTopo(vTopo, pType);
            if (vMapper.length() == 0)
            {
                iTops++;
            }
    ...
    bool FITKAbsGeoShapeAgent::isComponentValid(FITKModelEnum::FITKModelSetType compType, const QList<int> & members)
    {
        if (!_vtmanager)
        {
            return false;
